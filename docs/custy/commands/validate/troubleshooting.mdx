...
The selected provider is strict but configuration or `cz` is missing, the
configuration is malformed, or the final message violates project rules.

**How to resolve it**

Read the structured error label, then install `custy[commitizen]`, repair the
detected configuration, update the message, or select `provider = "custy"`.
Version strategy and message provider are independent. See the
