---
title: "Configuration Reference"
description: "Understand the project configuration file, precedence, and active sections."
sidebar_position: 2
sidebar_key: doc-gen-configuration
tags: [doc-gen, configuration, reference]
---

import {CommandHero, InfoBox, Section, Terminal} from "@site/src/components/docs";

<CommandHero command=".config/doc_gen/config.toml" title="Configuration Reference" description="Understand the project configuration file, precedence, and active sections." category="Configuration" status="Reference" syntax=".config/doc_gen/config.toml" />

| TOML table | Supported keys | Used by |
| --- | --- | --- |
| `tool.doc-gen.cli.execution` | `dry_run`, `debug` | Shared execution defaults |
| `tool.doc-gen.initialization` | `force`, `ask` | `doc-gen init` |
| `tool.doc-gen.cli.structure` | `target_directory`, `profile`, `smart_mode`, `max_depth`, `show_files`, `collapse_dirs`, `verbose` | All structure commands |
| `tool.doc-gen.cli.structure.generate` | `output_file` | `structure generate` |
| `tool.doc-gen.cli.structure.print` | Reserved; no active command-specific keys | `structure print` |
| `tool.doc-gen.cli.structure.analyze` | Reserved; no active command-specific keys | `structure analyze` |
| `tool.doc-gen.logging` | `level`, `markup`, `rich_tracebacks`, `show_time`, `show_level`, `console_format` | Console diagnostics |
| `tool.doc-gen.logging.file` | `level`, `log_dir`, `log_file`, `max_bytes`, `backup_count`, `file_format` | Rotating file diagnostics |

<Section title="Resolution order">

1. Explicit command-line argument
2. `.config/doc_gen/config.toml`
3. Internal default

</Section>

<InfoBox variant="note" title="Use active keys only">The `print` and `analyze` tables are placeholders for future command-specific settings. Put shared scan settings under `tool.doc-gen.cli.structure`.</InfoBox>
