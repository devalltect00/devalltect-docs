...
<CommandHero command="Doc Gen" title="Common Problems" description="Diagnose configuration, target, environment, and workflow failures." category="Support" status="Guide" syntax="doc-gen structure generate . --dry-run" />
...
<InfoBox variant="tip" title="Diagnostic first">Retry the smallest read-only command with global `--debug` or `--log-level DEBUG`, and verify the resolved target before live generation.</InfoBox>
