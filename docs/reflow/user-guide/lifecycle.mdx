---
title: "Lifecycle"
description: "This document explains the complete release lifecycle supported by Reflow including initialization, tag management, and container publishing."
sidebar_position: 6
sidebar_key: reflow-user-guide
---

# Release Lifecycle

## Overview

This document explains the complete release lifecycle supported by Reflow.

While each command can be used independently, Reflow is designed around a common workflow:

```text id="n7vj8r"
Install Reflow
       ↓
Initialize Project
       ↓
Configure Project
       ↓
Git Tags
       ↓
Release Automation
       ↓
Version Standardization
       ↓
Container Publishing
```

Understanding this lifecycle helps determine when each command should be used.

---

# Big Picture

```text id="n7c5h4"
Install Reflow
      │
      ▼
Initialize Project
      │
      ▼
Configure Project
      │
      ▼
Create Release Tags
      │
      ▼
Convert (Optional)
      │
      ▼
Replay (Optional)
      │
      ▼
Dockerize (Optional)
      │
      ▼
Container Registry
      │
      ▼
Deployment
```

Not every repository requires every step.

---

# Stage 1 — Initialize Project

Command:

```bash
reflow init
```

Purpose:

```text
Create configuration files and project scaffolding.
```

Typical output:

```text
config.toml
__version__.py
templates/
examples/
```

## Initialization Workflow

```text
Install Reflow
       ↓
reflow init
       ↓
Generate Project Files
       ↓
Edit Configuration
       ↓
Ready To Use Reflow
```

Documentation:

```text
docs/init/
```

This is usually the first command executed after installing Reflow.

# Stage 2 — Create Tags

Every workflow begins with Git tags.

Example:

```text id="vbk06j"
v1.0.0
v1.1.0
v2.0.0
```

or:

```text id="h4y6zq"
v2.0.0rc1
v2.0.0b1
v2.0.0a1
```

Tags represent release versions.

Reflow treats tags as the source of truth.

---

# Stage 3 — Replay Releases (Optional)

Command:

```bash id="e1r4z6"
reflow tags replay
```

Purpose:

```text id="9b6du6"
Recover release automation.
```

Typical scenarios:

```text id="svy0ii"
Missing GitHub releases
Failed release workflows
Repository migration
CI/CD migration
```

Workflow:

```text id="jzuvrx"
Tag Exists
      ↓
Release Missing
      ↓
Replay Tag
      ↓
Release Recreated
```

---

# Stage 4 — Convert Tags (Optional)

Command:

```bash id="j3ymwm"
reflow tags convert
```

Purpose:

```text id="3uxuzh"
Convert PEP 440 tags into SemVer tags.
```

Example:

Before:

```text id="q1h8w7"
v1.0.0rc1
v1.0.0b1
v1.0.0a1
```

After:

```text id="m0jj4m"
v1.0.0-rc.1
v1.0.0-beta.1
v1.0.0-alpha.1
```

This step is optional.

Repositories already using SemVer may skip conversion entirely.

---

# Stage 5 — Build Container Images

Command:

```bash id="p63j80"
reflow dockerize
```

Purpose:

```text id="36bvy8"
Turn release tags into Docker images.
```

Example:

Tag:

```text id="5w8v3s"
v2.0.0
```

Produces:

```text id="g1znq9"
ghcr.io/acme/my-app:v2.0.0
```

---

# Stage 6 — Publish Container Images

Dockerize pushes images into:

```text id="a0wb0l"
GitHub Container Registry
GitLab Container Registry
```

Example:

```text id="2c1l92"
ghcr.io/acme/my-app:v2.0.0
```

The image becomes available for deployment.

---

# Stage 7 — Deploy

Deployment occurs outside Reflow.

Examples:

```text id="jlwm2s"
Docker Compose
Kubernetes
Nomad
Azure
AWS
Google Cloud
```

Reflow stops after publishing images.

---

# Common Workflows

## New Project Workflow

Typical workflow:

```bash
reflow init
```

Review configuration:

```text
config.toml
```

Then run the commands you need:

```bash
reflow tags convert

reflow tags replay

reflow dockerize
```

## Release Recovery Workflow

Problem:

```text id="8k4ej0"
Tags exist.
Releases missing.
```

Workflow:

```bash id="0z4iqz"
reflow tags replay
```

Result:

```text id="t5a3uz"
Release automation re-triggered.
```

---

## Version Migration Workflow

Problem:

```text id="qltmbz"
PEP 440 tags.
```

Need:

```text id="7hch2h"
Semantic Versioning.
```

Workflow:

```bash id="9rj4b4"
reflow tags convert
```

Result:

```text id="p4n75l"
SemVer tags created.
```

---

## Container Publishing Workflow

Problem:

```text id="p2eh1j"
Tags exist.
Images missing.
```

Workflow:

```bash id="mjlwm8"
reflow dockerize
```

Result:

```text id="hmh34j"
Images published.
```

---

## Full Migration Workflow

```text
Initialize Project
        ↓
Configure Project
        ↓
Convert Tags
        ↓
Replay Releases
        ↓
Dockerize Images
```

Typical workflow:

```bash id="iqaz2s"
reflow tags convert
```

```bash id="q8j7ey"
reflow tags replay
```

```bash id="cb3t1o"
reflow dockerize
```

Workflow:

```text id="e7z72l"
PEP 440
      ↓
SemVer
      ↓
Release Recovery
      ↓
Docker Images
```

---

# When To Use Each Command

## Replay

Use when:

```text id="njh8v2"
Release automation failed.
```

---

## Convert

Use when:

```text id="h6cr35"
Tag format migration is needed.
```

---

## Dockerize

Use when:

```text id="mjqh1k"
Deployable container images are needed.
```

---

# When NOT To Use Each Command

## Replay

Do not use when:

```text id="sxqrv5"
You need new versions.
```

Replay does not create versions.

---

## Convert

Do not use when:

```text id="e1u0l8"
Tags already follow SemVer.
```

---

## Dockerize

Do not use when:

```text id="9a0j2n"
No Dockerfile exists.
```

or:

```text id="o56l6j"
Container images are not required.
```

---

# Current Supported Lifecycle

Current support:

```text id="dlzvxt"
Git Tags
    ↓
Replay
    ↓
Convert
    ↓
Dockerize
    ↓
GHCR / GitLab Registry
```

---

# Future Lifecycle Possibilities

Potential future enhancements:

```text id="s5r5j0"
Docker Hub
AWS ECR
Azure ACR
Google Artifact Registry

Single Tag Processing
Tag Range Processing

SemVer → PEP440

Release Report Generation
```

---

# Summary

Reflow treats Git tags as the central release artifact.

Everything else flows from those tags:

```text id="jzlp9v"
Initialize Project
       ↓
Configure Project
       ↓
Git Tags
       ↓
Release Recovery
       ↓
Version Standardization
       ↓
Container Publishing
```

This philosophy keeps release automation predictable, repeatable, and easy to understand.
