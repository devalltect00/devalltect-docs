...
export function getTopics(packageData, providerKey) {
  ...
  if (!Array.isArray(configured)) {
    throw new Error(`package.json "devalltect.${providerKey}.topics" must be an array`);
  }
...
export function normalizeRepositoryUrl(remoteUrl, provider) {
  ...
  const acceptedHosts = new Set(
    [provider.hostname, ...provider.hostAliases].map((host) => host.toLowerCase())
  );
  ...
  if (
    segments.some(
      (segment) =>
        ["", ".", "..", "-"].includes(segment) || !/^[A-Za-z0-9_.-]+$/u.test(segment)
    )
  ) {
...
export function runApiRequest(
  ...
  if (result.error) {
    throw new Error(`unable to run ${request.executable}: ${result.error.message}`);
  }
  if (result.status !== 0) {
    throw new Error(
...
export function main(argv = process.argv.slice(2)) {
  ...
  const providers =
    options.provider === "all" ? [GITHUB, GITLAB] : [PROVIDERS[options.provider]];
  ...
  for (const plan of plans) {
    ...
    if (plan.provider.key === GITHUB.key && plan.topics.length > GITHUB_TOPIC_LIMIT) {
      console.log(
