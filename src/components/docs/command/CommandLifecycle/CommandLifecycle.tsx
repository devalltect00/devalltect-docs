...
import type { CommandLifecycleProps } from "./types";
...
export default function CommandLifecycle({
  ...
  return (
    <section className={clsx(styles.lifecycle, className)} {...props}>
      <Heading
        level={2}
        title={title}
      ...
      <Card>
        <ol className={styles.timeline}>
          {steps.map((step, index) => (
            <li
              key={step.title}
              className={clsx(styles.step, step.active && styles.active)}
            >
              ...
              <div className={styles.content}>
                <div className={styles.header}>
                  ...
                  {step.active && <Badge variant="primary">Current</Badge>}
                </div>
